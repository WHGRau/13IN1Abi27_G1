...
public class Bibliothek {
    ...
    public void buchLeihen(int ausleihZeitTage) {
        if (isLehrer()) {
            ...
            for (int i = 0; i < erfassteBuecher.size(); i++) {
                ...
                dbConnector
                        .executeStatement("SELECT status, anzahlLiehen FROM buecher WHERE isbn = '" + erfassteBuecher.get(i) + "'");
                QueryResult result = dbConnector.getCurrentQueryResult();
                if(result != null && result.getRowCount() > 0){
                    if (result.getData()[0][0].equals("verfuegbar") || result.getData()[0][0].equals("reserviert")) {
                        ...
                        dbConnector.executeStatement("SELECT id, status FROM reservierungen WHERE schueler_id = '" + erfassterSchueler + "' AND isbn = '"
                                + erfassteBuecher.get(i) + "' AND (status = 'bereit' OR status = 'wartend') ORDER BY id DESC LIMIT 1");
    ...
    public void buchHinzufuegen(String isbn, String titel, String autor, int jahr, String beschreibung) {
        if (isLehrer()) {
            ...
            QueryResult result = dbConnector.getCurrentQueryResult();
            if(result == null || result.getRowCount() == 0){
                String sql = "INSERT INTO buecher (isbn, titel, autor, erscheinungsjahr, beschreibung, status, anzahlDa, anzahlLiehen, anzahlRes)" + " VALUES('"
                    + isbn + "', '" + titel + "', '" + autor + "'," + jahr + ",'" + beschreibung + "','verfuegbar', 1, 0, 0)";
                dbConnector.executeStatement(sql);
            }
        }
    }
        ...
        if (isLehrer()) {
            ...
            if (result != null && result.getRowCount() > 0) {
                ...
                if(maxRes(isbn)){
                    ...
                    if (result.getData()[0][0].equals("0")){
                        loeschRE(isbn);
        ...
        if (isLehrer()) {
            try {
                ...
                dbConnector.executeStatement("SELECT id, status FROM reservierungen WHERE isbn = '" + isbn
                        + "' AND schueler_id = " + schuelerId + " AND (status = 'bereit' OR status = 'wartend') LIMIT 1");
                QueryResult result = dbConnector.getCurrentQueryResult();
                if (result != null && result.getRowCount() > 0) {
    ...
    public int getTageZuSpaet(String isbn) {
        if(erfassterSchueler != null){
            dbConnector.executeStatement("SELECT DATEDIFF(CURRENT_DATE(), geplante_rueckgabe) FROM ausleihen WHERE isbn = '"
                + isbn + "' AND ruckgabe_datum IS NULL AND schueler_id = '" + erfassterSchueler+"'");
            QueryResult result = dbConnector.getCurrentQueryResult();
            if (result != null && result.getRowCount() > 0) {
    ...
    public void reservierungenAktualisieren() {
        ...
        if (result != null && result.getRowCount() > 0) {
            for (int i = 0; i < result.getRowCount(); i++) {
                ...
                hinzuDA(result.getData()[i][1]);
                loeschRE(result.getData()[i][1]);
                updateBuchStatus(result.getData()[i][1]);
    ...
    public ArrayList<String> checkBuecherReserviert() {
        ...
        for (int i = 0; i < erfassteBuecher.size(); i++) {
            String isbn = erfassteBuecher.get(i);
            dbConnector.executeStatement("SELECT schueler_id, buecher.anzahlDa FROM reservierungen, buecher WHERE reservierungen.isbn = '"
                    + isbn + "' AND status = 'bereit' AND buecher.isbn = reservierungen.isbn");
            QueryResult result = dbConnector.getCurrentQueryResult();
            if (result != null && result.getRowCount() > 0) {
    ...
    public void letzteAktionZuruecknehmen() {
        if (letzteAktionAusleihen) {
            for (String isbn : letzteBuecher) {
                ...
                dbConnector.executeStatement("SELECT id FROM reservierungen WHERE isbn = '" + isbn
                        + "' AND schueler_id = " + letzterSchueler + " AND status = 'abgeschlossen' ORDER BY id DESC LIMIT 1");
            ...
            for (String isbn : letzteBuecher) {
                ...
                hinzuLI(isbn);
                dbConnector.executeStatement("SELECT COUNT(id) FROM reservierungen WHERE isbn = '"+isbn+"' AND status = 'bereit'");
                QueryResult result = dbConnector.getCurrentQueryResult();
                if(result != null && result.getRowCount() > 0){
    ...
    public boolean bereitsAusgeliehen(String isbn){
        dbConnector.executeStatement("SELECT id FROM ausleihen WHERE isbn = '"+isbn+"'AND schueler_id ='" + erfassterSchueler +"' AND ruckgabe_datum IS NULL");
        QueryResult result = dbConnector.getCurrentQueryResult();
        if(result == null || result.getRowCount() == 0){
            return false;
        }
